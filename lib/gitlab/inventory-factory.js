...
class GitLabInventoryFactory extends AbstractInventoryFactory {
  ...
  fetchCodeJsonReleases(project) {
    const url = [
      ...
      "/api/v4/projects/",
      project.id,
      "/repository/files/code.json/raw?ref=",
      project.default_branch,
    ].join("");
    const httpOpts = {
      headers: {
        "PRIVATE-TOKEN": this.endpoint.password,
      },
    };
    ...
    return new Promise((resolve, reject) => {
      ...
      https
        .get(url, httpOpts, (response) => {
          if (response.statusCode !== 200) {
            return reject(
              new Error(
                `Download for "${url}" resulted in HTTP status ${response.statusCode}`
              )
            );
          }
